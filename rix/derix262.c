...
static void *modern_timer_loop(void *arg)
{
	unsigned long long next_tick_ns;
	(void)arg;
	if(g_tick_ns == 0) g_tick_ns = (65536ULL * 1000000000ULL) / 1193180ULL;

	#if defined(__MINGW32__)
	next_tick_ns = (unsigned long long)GetTickCount64() * 1000000ULL;
	#else
	{
		struct timespec now;
		clock_gettime(CLOCK_MONOTONIC, &now);
		next_tick_ns = ((unsigned long long)now.tv_sec * 1000000000ULL) + (unsigned long long)now.tv_nsec;
	}
	#endif
	...
	while(g_timer_running)
	{
		...
		if(ns == 0) ns = (65536ULL * 1000000000ULL) / 1193180ULL;
		next_tick_ns += ns;

		#if defined(__MINGW32__)
		while(g_timer_running)
		{
			unsigned long long now_ns = (unsigned long long)GetTickCount64() * 1000000ULL;
			unsigned long long remain_ns;
			DWORD sleep_ms;
			if(now_ns >= next_tick_ns) break;
			remain_ns = next_tick_ns - now_ns;
			sleep_ms = (DWORD)(remain_ns / 1000000ULL);
		...
		#else
		{
			...
			target.tv_sec = (time_t)(next_tick_ns / 1000000000ULL);
			target.tv_nsec = (long)(next_tick_ns % 1000000000ULL);
