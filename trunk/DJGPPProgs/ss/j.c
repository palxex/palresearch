...
byte* seeksub(byte* x,int n,int max,int *width,int *height)
{
	int i=0,r=0;
	while(i<n){
		r+=mget16(x+i*4+2)*mget16(x+i*4+4);
		i++;
	}
	*width=mget16(x+n*4+2);
	*height=mget16(x+n*4+4);
	return x+mget16(x)*4+2+r;
}
...
{	
	...
	*(strrchr(prefix,'.'))='\0';
	...
	{		
		...
		textprintf_ex(screen,font,0,0,0xFF,0,"group:%x,id:%x,pat:%x,width:%x,height:%x",mapgroup,map,pat,width[map],height[map]);			
